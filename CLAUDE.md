...
- **`rag_quest/web/__init__.py` shadows the `rag_quest.web.app` submodule** by re-exporting the FastAPI `app` instance as an attribute. Tests that need to monkeypatch module-level helpers in `web/app.py` (e.g. `_resolve_save_path`, `_write_save_file`) must use `importlib.import_module("rag_quest.web.app")`. `from rag_quest.web import app as web_app` silently gives you the FastAPI instance instead of the module, and `monkeypatch.setattr` fails with `AttributeError: FastAPI object has no attribute '_resolve_save_path'`.
- **`SaveManager.save_game` always creates a new UUID slot** (`rag_quest/saves/manager.py:112`) — there is no in-place overwrite. A web `/save` endpoint must write directly to the existing slot path. Also: two save layouts coexist and `session_id` means different things in each flow: the CLI + `/session/new` onboarding use flat `saves/{world_name}.json` with `session_id = world.name`, while `/session/load` uses slot-dir `saves/{uuid}/state.json` with `session_id = uuid`. Any session-id-based path resolver has to check both. Partially tracked by `rag-quest-dbs`; the two-layouts trap is the part that bites resolvers.
